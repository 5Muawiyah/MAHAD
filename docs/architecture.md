...
The worker polls the active symbol every five seconds (`config.POLL_INTERVAL_S`) from a timer on its own thread; the threading section below records the switch defect that stops that timer. The adapter returns a `FetchResult` whose `error` field carries a failure as a value; nothing in the data layer raises across the boundary.
...
Intents travel the other way as queued slots on the worker: add and remove a symbol, change the indicator settings, place a simulated order, reset the portfolio, clear the trade log, arm, disarm, re-arm and remove alerts, change the risk timeframe, and retry a poll (the chart's Retry button, which also clears the backoff). Selects and timeframe clicks are coalesced in the window through a 250 ms single-shot timer so a burst of clicks becomes one fetch. Those two, `select_symbol` and `set_timeframe`, are plain method calls from that timer's slot in `main_window.py`, so they run on the window thread: the active-symbol write to SQLite and, for a stock with no cached history, the Tiingo pull happen there, and the one-shot poll they schedule runs on the window thread too. Both methods also restart the worker's five-second poll timer, and Qt refuses to stop or start a timer from another thread (it logs "Timers cannot be stopped from another thread" and the timer is left stopped), so after a switch from the window the periodic poll stays stopped until a later queued intent restarts the timer from the worker thread (removing the active symbol, or adding the first symbol to an emptied watchlist, both of which reach `_switch_active` on that thread), and the next switch from the window stops it again. While it is stopped the active symbol and the held marks are fetched once per switch and once per press of the Retry button, alerts are evaluated only on those one-shot polls (`_poll` is their only caller) rather than on the five-second grid, and the heartbeat timer, which the switch never touches, carries on. Until the two calls are routed through queued signals like the other intents, which is the fix, the one-shot poll and the active-symbol write use the worker's session from the window thread while the heartbeat may be using it on the worker thread.
...
Shutdown is cooperative and bounded. `MainWindow.closeEvent` requests interruption on the thread, emits a queued stop, quits the thread and waits for a budget that covers a worst legal poll cycle (two request timeouts, three held-mark timeouts and five seconds) before terminating. The queued stop is best effort: `quit` follows the emit at once and usually ends the thread's loop before the posted call is delivered, so the interruption flag, which in-flight and queued work checks between steps, is what ends the work, and the repository is closed by process exit rather than by `PollWorker.stop`.
