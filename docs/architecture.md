...
Three read-model modules live under `mahad/data/` and are the only data modules the window imports: `symbols.py` (watchlist and alert rows), `portfolio_view.py` (portfolio, risk and analytics views) and `context_view.py` (context tiles, health and data-integrity views). They are frozen dataclasses with no logic beyond a few string helpers. The window never computes a figure; it renders what the worker publishes and sends intents back.

The engine is Qt-free and the report module (`mahad/report.py`) imports the engine and the data layer only. A layering check in the maintainer's tooling walks every import under `mahad/` and fails on any crossing.

## The path of a price

The worker polls the active symbol every five seconds (`config.POLL_INTERVAL_S`). The adapter returns a `FetchResult` whose `error` field carries a failure as a value; nothing in the data layer raises across the boundary.
...
Two kinds of channel run from the worker to the window. The snapshot channel (`snapshot_ready`, `watchlist_ready`, `alerts_ready`) carries complete state on every emission, so whichever emission arrives last is the one worth rendering; the chart also discards a snapshot for a symbol the user has already switched away from, through its pending-switch guard. The discrete channels (`alert_events`, `order_result`, `add_rejected`, `remove_rejected`, `alert_rejected`, `settings_applied`, `db_notice`) carry one-off outcomes that must never be merged or lost: an alert firing, an order filling or being refused, a rejected symbol, a database that could not be opened.

Intents travel the other way as queued slots on the worker: add, remove and select a symbol, change the timeframe or the indicator settings, place a simulated order, reset the portfolio, arm, disarm, re-arm and remove alerts, change the risk timeframe. Selects and timeframe clicks are coalesced in the window through a 250 ms single-shot timer so a burst of clicks becomes one fetch.
...
Shutdown is cooperative and bounded. `MainWindow.closeEvent` requests interruption on the thread, emits a queued stop, quits the thread and waits for a budget that covers a worst legal poll cycle (two request timeouts, three held-mark timeouts and five seconds) before terminating. In-flight and queued work checks the interruption flag between steps.
...
The headless report is the documented exception to the single-writer rule, and it keeps the rule intact by never writing. `mahad/report.py` opens the same file through the repository with an SQLite URI of the form `sqlite:///file:<path>?mode=ro&uri=true`. In that mode SQLite refuses every write, `CREATE TABLE` included, so the repository's start-up path only reads the schema-version row; a missing file is an error and is never created. WAL mode is what makes this safe while the application is running: readers and the one writer do not block each other, and a read sees a consistent snapshot of the file. The decision was taken over the alternative of a second write-capable session because two writers on one SQLite file would need cross-process coordination that the application has no reason to carry.
...
`mahad/engine/` is pure Python with pandas and numpy: indicators, alert signals, the exact-Decimal ledger, the wall-clock risk metrics, the trading-day return series and analytics, the NYSE calendar, resampling and the snapshot builder. Nothing in it imports Qt, and the tests import it directly. Three modules carry the risk formulas: `risk.py` (exposure, volatility and maximum drawdown on the value history), `returns.py` (the aligned daily return series and the as-if portfolio series) and `risk_metrics.py` (VaR, Expected Shortfall, the Kupiec test and the Basel zones, beta, Sharpe, Sortino, EWMA, correlation, concentration, drawdown duration, stress replay, component VaR). Each formula is pinned to a hand-worked vector in the [verification note](verification.md) by `tests/test_risk.py`, `tests/test_returns.py` and `tests/test_risk_metrics.py`, and the conventions those vectors lock in (order statistics rather than interpolation, `ddof = 1`, no SciPy, the Euler identities) are stated in the [methodology note](risk-methodology.md).
...
| `mahad/data/context_sources.py` | Treasury, Fear and Greed, VIX, Frankfurter and Bank of England fetchers; `.env` key loading |
