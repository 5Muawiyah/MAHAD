...
The chart of the active symbol (the short code for a share or a crypto pair, such as AAPL or BTC/USD) fills the left, price in the corner (here Apple at 312.06 USD, up 0.59%) and, in the toolbar above it beside the symbol name, the 1m to 1mo buttons (one minute to one month) setting the span of each bar; the watchlist and risk panel sit on the right. The portfolio tab at the bottom shows cash, total value, and profit and loss split into realised (on trades already closed) and unrealised (on positions still held).

![Price chart with SMA, EMA and RSI](images/chart.png)

The chart draws the closing price with a 20-bar simple and a 12-bar exponential moving average over it, and below it the 14-bar relative strength index, a 0 to 100 gauge read as overbought above 70 and oversold below 30. Every line is drawn on completed bars only.
...
The top of the panel shows exposure (how much of the portfolio's value is in positions rather than cash), the volatility (the typical size of one period's move, a day by default, as a percentage) of the portfolio's own value, and its worst fall from a peak. The analytics section below runs the measures such teams use, most on the last 250 trading days and the correlation grid on the last 90. The loss measures are [Value-at-Risk](glossary.md) at two confidence levels, Expected Shortfall, their bell-curve (parametric) versions, the traffic-light backtest and three replayed shocks such as the 2020 COVID crash. Others describe reward and shape: [beta](glossary.md) against the S&P 500 fund SPY, the [Sharpe](glossary.md) and [Sortino](glossary.md) ratios, a recent-weighted (EWMA) volatility, [concentration](glossary.md), each position's share of the risk, and a grid of how closely the holdings move together.

![The risk panel with the market context section open](images/market-context.png)

The market context section gives the backdrop: the US Treasury 10-year yield (what the bond pays a year) and the gap between two-year and ten-year yields (35 basis points, or 0.35 percentage points, a normal slope), the VIX fear gauge, the crypto Fear & Greed index (9, extreme fear), and the two UK rates, SONIA (the sterling overnight rate) and the Bank Rate.
...
Historical VaR reads the figure from what happened instead: with 100 days of returns at 95%, the program sorts the days from worst to best and takes the sixth worst, 2.50% in the worked example. Expected Shortfall asks how bad the bad days are on average: it averages the six worst days, 3.4667% in the same example, and is never smaller than the VaR it pairs with.
...
Drawdown is the fall from a peak: a value that goes 100, 110, 99, 104.5, 112, 108 has a maximum drawdown of minus 10%, the fall from 110 to 99. Concentration uses the Herfindahl-Hirschman Index, the sum of the squared weights: 50%, 30% and 20% give 0.25 + 0.09 + 0.04, or 0.38, and one over that is about 2.6 equal positions. Beta says how much the portfolio moves with the market: 1.16 means a 1% market move tends to bring a 1.16% move. Sharpe divides the return above a risk-free rate (the three-month US Treasury bill yield) by the variation it took to earn it; Sortino divides the plain return by the downside variation only.
