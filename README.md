...
On the simulated portfolio the panel computes [Value-at-Risk](docs/glossary.md) (the loss a bad day could reach, read from past days or from a bell curve), Expected Shortfall (the average loss on the days at or beyond it) and each holding's share of the total risk (component VaR). Beta says how much the portfolio moves with the market, and the Sharpe and Sortino ratios give the return per unit of variation. A smoothed volatility (EWMA, an exponentially weighted moving average that leans on recent days), concentration (how much sits in the largest holdings) and a correlation heatmap (how closely the holdings move together) describe how the portfolio is spread. A backtest counts how often losses beat the VaR and shows the result as the Basel traffic light, the supervisors' green, yellow and red reading, with the Kupiec statistical check in the row's tooltip. Replays of dated shocks such as the 2020 COVID crash show what past crises would have cost. Every figure is labelled with the period it covers (its window) and the price history it is measured on (its basis), so the same holding's different percentages are never ambiguous.
...
Copy `.env.example` to `.env` and paste your keys in. The `.env` file is never committed and never leaves your machine, and none of the keys can trade or move money. MAHAD uses the FRED API but is not endorsed or certified by the Federal Reserve Bank of St. Louis.
